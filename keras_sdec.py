...
class GCN(Layer):
    def __init__(self, output_dim, input_dim, activation='relu', weight_initializer='glorot_uniform', bias_initializer='zeros',**kwargs):
    ...
    def build(self, input_shape):
        ...
        # Weights
        self.W = self.add_weight("W", shape=[input_dim, self.output_dim], initializer=self.weight_initializer)
        # bias
        self.bias = self.add_weight("bias", shape=[self.output_dim], initializer=self.bias_initializer)

    def call(self, AX):
        return self.activation(K.dot(AX, self.W) + self.bias)
...
class SDEC:
    ...
    @staticmethod
    def add_loss(Z, X_, X, Y, lambd=1e-5):
        n = Z.shape[0]
        ...
        res = 0
        xent_loss=np.mean((X_-X)**2)
        return res * lambd / n + xent_loss

    @staticmethod
    def get_pairwise_constraints(Y):
        Y = Y.reshape(-1, 1)
        # a = np.where(Y.dot(Y.T) == np.square(Y), 1, -1) #原来的
        a = np.where(Y.dot(Y.T) == np.square(Y), 1, 0)
...
class DeepEmbeddingClustering(object):
    ...
    def initialize(self, X, save_autoencoder=True, layerwise_pretrain_iters=50000, finetune_iters=100000):
        if self.pretrained_weights is None:
            ...
            self.autoencoder.load_weights(self.pretrained_weights+'/autoencoder.h5')
        ...
        self.x_input = Input(shape=(self.input_dim,), dtype='float32',name='x')
        self.x_encoder = self.encoder(self.x_input)

        self.Ax_input = Input(shape=(self.input_dim,), dtype='float32',name='Ax')
        self.Ax_GCN = GCN(output_dim=10,input_dim=self.input_dim)(self.Ax_input) #!!
        x = keras.layers.add([self.x_encoder, self.Ax_GCN])
        self.ClusteringLayer = ClusteringLayer(self.n_clusters,
                        weights=self.cluster_centres,
                        name='clustering')(x)
        self.DEC = Model([self.x_input, self.Ax_input],self.ClusteringLayer)

        if self.pretrained_weights is not None:
            self.DEC.load_weights(self.pretrained_weights+'/DEC.h5')
    ...
    def cluster(self, X, AX, y=None,
        ...
        while train:
            ...
            if iteration % update_interval == 0:
                self.q = self.DEC.predict([X,AX], verbose=0)
                ...
                if y is not None:
                    ...
                    # !!
                    NMI = nmi_score(y_pred, y)
                    from sklearn.metrics.cluster import adjusted_rand_score
                    ARI = adjusted_rand_score(y, y_pred)
                    self.accuracy.append(acc)
                    print('Iteration '+str(iteration)+', Accuracy'+str(np.round(acc, 5)),', NMI '+str(np.round(NMI, 5)),', ARI '+str(np.round(ARI, 5)))
                ...
                for i in range(len(self.encoder.layers)):
                    self.encoder.layers[i].set_weights(self.DEC.layers[2].layers[i].get_weights())#!!
            ...
            if (index+1)*self.batch_size > X.shape[0]:
                ...
                self.DEC.loss = SDEC.sdec_loss(
                    add_loss=0)
                loss = self.DEC.train_on_batch([X[index*self.batch_size::],AX[index*self.batch_size::]], self.p[index*self.batch_size::])
                index = 0
                ...
                self.DEC.loss = SDEC.sdec_loss(
                    add_loss=0)
                loss = self.DEC.train_on_batch([X[index*self.batch_size:(index+1) * self.batch_size],AX[index*self.batch_size:(index+1) * self.batch_size]],
